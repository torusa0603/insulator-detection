...
class DataFromTelemetry():
    def __init__(self):
        ...
        self.objectdetection_mode = CST.OBJECTDETECTION_MODE.default_center
        self.current_aircraft_waypoint = -1
        self.confidence_bias = 0
        self.objectdetection_tower_mode = CST.OBJECTDETECTION_TOWER_MODE.taicho_220kv

gbl_data_from_telemetry = DataFromTelemetry()

class InstructionFromClick():
    currentCameraLens = CST.CAMERALENS.wide
    targetSquareWidth = 120
    targetSquareHeight = 120

gbl_instruction_from_click = InstructionFromClick()

class GimbalInstructionFromDetection():
    zoomValueCurrent = 2
    gimbalCornerX = 0
    gimbalCornerY = 0
    relatedX_old = 0
...
class SavePrevData():
    objectdetection_status_old = CST.OBJECTDETECTION_STATUS.no_action
...
class TerraUTMSDKMainNode():
    ...
    def __init__(self):
        ...
        self.const_setting_value = ConstSettingValue(
            target_model_class_indexes=[CST.CLASS_INSULATOR.t_660, CST.CLASS_INSULATOR.t_, CST.CLASS_INSULATOR.t_500], 
            input_mode = CST.INPUT_MODE_GSTREAMER.udp,
            interval = CST.INTERVAL.version_202203,
            objectdetection_target_model = CST.OBJECTDETECTION_TARGET_MODEL.insulator,
            model_path = CST.MODEL_PATH,
            output_path = CST.OUTPUT_PATH,
            maxCornerX = 12, 
            maxCornerY = 7, 
    ...
    def telemetryCallback(self, data):
        ...
        try:
            ...
            if data.objectdetection_mode == 0:
                gbl_data_from_telemetry.objectdetection_tower_mode = CST.OBJECTDETECTION_TOWER_MODE.taicho
            ...
            print("gimbal_pitch: {}, gimbal_roll: {}, gimbal_yaw: {}, gimbal_mode: {}, gimbal_mode_old: {}, mission_status: {}, MISSION_STATUS_PAUSE: {}, objectdetection_status: {}".format(gbl_gimbal_control.pitch,gbl_gimbal_control.roll,gbl_gimbal_control.yaw,gbl_gimbal_control.mode,gbl_gimbal_control.mode_old,mission_status,CST.MISSION_STATUS.pause,objectdetection_status))

            if (objectdetection_status != gbl_save_prev_data.objectdetection_status_old):
                print("モード切替！")
                if(objectdetection_status == CST.OBJECTDETECTION_STATUS.start): # OD開始
                    #物体検知処理有効
                    gbl_data_from_telemetry.flg_objectdetection_auto = True
                    gbl_data_from_telemetry.objectdetection_mode = CST.OBJECTDETECTION_MODE.default_center
                    #ジンバル角度保存
                    gbl_gimbal_control.pitch_saved = gbl_gimbal_control.pitch
                    ...
                    pass
                elif(objectdetection_status == CST.OBJECTDETECTION_STATUS.no_action) : # OD無効
                    #物体検知処理無効
                    self.gimbalcornerReset()
                    self.cameraControlZoom(CST.ZOOM_VALUE.Val_2)
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def on_click(event, x, y, flags, frame):
        if event == cv2.EVENT_LBUTTONUP:
            ...
            filename = "{}/{}.jpg".format(CST.OUTPUT_PATH, date_str)
    ...
    def onMouseClick(self,event,x,y,flags,param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if x >= gbl_camera_control.width // 2 - gbl_instruction_from_click.targetSquareWidth // 2 and x <= gbl_camera_control.width // 2 + gbl_instruction_from_click.targetSquareWidth // 2 and y >= gbl_camera_control.height // 2 - gbl_instruction_from_click.targetSquareHeight // 2 and y <= gbl_camera_control.height // 2 + gbl_instruction_from_click.targetSquareHeight // 2 :
                ...
                #レンズ切り替え
                if (gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.dummy_pause) :
                    #処理再開
                    print("objectdetection continue(change lens(Pause->Zoom))")
                    gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.zoom
                elif gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.wide :
                    #処理一時停止
                    print("objectdetection pause(change lens(Wide->Pause))")
                    gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.dummy_pause
                elif gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.zoom :
                    #処理一時停止
                    print("objectdetection pause(change lens(Wide->Pause))")
                    gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.dummy_pause
                    ...
                    gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.wide
                ...
                #カメラ切替
                if gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.wide:
                    gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.zoom
                    self.cameraControlChangeLens(CST.OSDK_CAMERA_SOURCE_H20T.zoom)
                else:
                    gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.wide
                    self.cameraControlChangeLens(CST.OSDK_CAMERA_SOURCE_H20T.wide)
                ...
                #x4
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_4)
            elif x >= 0 and x <= 60 and y >= 200 and y <= 240 :
                #x5
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_5)
            elif x >= 0 and x <= 60 and y >= 240 and y <= 280 :
                #x6
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_6)
            elif x >= 0 and x <= 60 and y >= 280 and y <= 320 :
                #x8
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_8)
            elif x >= 0 and x <= 60 and y >= 320 and y <= 360 :
                #x10
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_10)
            elif x >= 0 and x <= 60 and y >= 360 and y <= 400 :
                #x12
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_12)
            elif x >= 0 and x <= 60 and y >= 400 and y <= 440 :
                #x16
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_16)
            elif x >= 0 and x <= 60 and y >= 440 and y <= 480 :
                #x20
                self.cameraControlZoom(CST.ZOOM_VALUE.Val_20)
            else : 
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def gimbalcornerReset(self):
        ...
        gbl_gimbal_instruction_from_detection.zoomValueCurrent = CST.ZOOM_VALUE.Val_2                #ズーム初期化

        gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.wide #レンズはwideで初期化
        ...
        try :
            ...
            for det in input_prob:
                ...
                if confidence > gbl_data_from_telemetry.confidence_bias :
                    ...
                    print("[class:{}], confidence:{:.1f} x1:{}, y1:{}, x2:{}, y2:{},boundingbox_width:{},boundingbox_height:{}".format(CST.DISPLAY_ATTRIBUTES.classes[index], confidence, x1, y1, x2, y2, boundingbox_width, boundingbox_height))
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def objectdetection(self):
        ...
        gbl_save_prev_data.flg_objectdetection_auto_prev = False

        while True :
            try :
                print("start cap")
                if self.const_setting_value.input_mode == CST.INPUT_MODE_GSTREAMER.uri:
                    print("INPUT_MODE:{}, VALUE:{}".format(CST.INPUT_MODE_GSTREAMER.udp,CST.INPUT_MODE_GSTREAMER.uri_str))
                    cap = cv2.VideoCapture(CST.INPUT_MODE_GSTREAMER.uri_str,cv2.CAP_GSTREAMER)      #gstreamer_uriから
                else :
                    print("INPUT_MODE:{}, VALUE:{}".format(CST.INPUT_MODE_GSTREAMER.udp,CST.INPUT_MODE_GSTREAMER.udp_str))
                    cap = cv2.VideoCapture(CST.INPUT_MODE_GSTREAMER.udp_str,cv2.CAP_GSTREAMER)      #gstreamer_udpから
                print("end cap")
                if not cap.isOpened():
                    sys.exit()
        ...
        print("cap test")
        #モデル読み込み
        model = Model(CST.DISPLAY_ATTRIBUTES.classes, CST.DISPLAY_ATTRIBUTES.colors, height, width, CST.PATH.model)
        ...
        while not rospy.is_shutdown():
            ...
            if (gbl_data_from_telemetry.flg_objectdetection_auto == True ) :
                ...
                if gbl_save_prev_data.flg_objectdetection_auto_prev == False :
                    #物体検知開始
                    ...
                    if gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.taicho_wide_to_zoom:
                    ...
                    gbl_save_prev_data.flg_objectdetection_auto_prev = True
                    continue 
                    

                if (counter % self.const_setting_value.interval == 0):
                    # モード判定
                    if gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.taicho_220kv:
                        if gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.wide:
                            print("物体開始時ズーム初期化(2倍)")
                            self.cameraControlZoom(CST.ZOOM_VALUE.Val_2)
                            gbl_instruction_from_click.currentCameraLens = CST.CAMERALENS.zoom
                            ...
                            continue
                    elif gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.taicho_wide_to_zoom:
                        if gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.zoom:
                            #ズームのとき
                    ...
                    if label == CST.CAMERALENS.dummy_pause : 
                    ...
                    if len(generate_sampling_group_data) > 0 :
                        ...
                        for i in range(len(generate_sampling_group_data)):
                            ...
                            if gbl_data_from_telemetry.objectdetection_mode == CST.OBJECTDETECTION_MODE.default_center :           #中心優先
                                print("中心優先")
                                target_detected_index = target_index_min_detection_distance
                            elif  gbl_data_from_telemetry.objectdetection_mode == CST.OBJECTDETECTION_MODE.default_area :          #面積優先
                                print("面積優先")
                                target_detected_index = target_index_max_detection_square
                            elif  gbl_data_from_telemetry.objectdetection_mode == CST.OBJECTDETECTION_MODE.confidence :    #認識率優先
                                print("認識率優先")
                            ...
                            if (int(det[0]) in self.const_setting_value.target_model_class_indexes and i == target_detected_index and flg_control_gimbal == True):   #test20220310
                                ...
                                if det[4] <  gbl_instruction_from_click.targetSquareWidth and ((gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.zoom and gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.taicho_wide_to_zoom) or (self.const_setting_value.flg_target_strict_judgement == True)) :
                                ...
                                if det[5] <  gbl_instruction_from_click.targetSquareHeight and ((gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.zoom and gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.taicho_wide_to_zoom) or (self.const_setting_value.flg_target_strict_judgement == True)) :
                                ...
                                if ((det[2] > gbl_gimbal_instruction_from_detection.centerX - (tmp_targetSquareWidth // 2 * tmpzoom) and det[2] < gbl_gimbal_instruction_from_detection.centerX + (tmp_targetSquareWidth //2 * tmpzoom)  \
                                    ...
                                    (gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.kensui_500kv and gbl_gimbal_control.pitch > 28 and det[2] > gbl_gimbal_instruction_from_detection.centerX - (tmp_targetSquareWidth // 2 * tmpzoom) and det[2] < gbl_gimbal_instruction_from_detection.centerX + (tmp_targetSquareWidth //2 * tmpzoom)) \
                                    ): #test20211210 change #20220310 add

                                #changeend 20220401
                                    print("中心！")
                                    if ( gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.wide and gbl_data_from_telemetry.objectdetection_tower_mode != CST.OBJECTDETECTION_TOWER_MODE.taicho_wide_to_zoom):
                                    ...
                                    if (det[4] >= gbl_camera_control.width * self.const_setting_value.zoom_bias or det[5] >= gbl_camera_control.height * self.const_setting_value.zoom_bias):
                                        ...
                                        #物体検知のモード判定
                                        if gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.taicho_220kv:
                                            self.cameraControlZoom(CST.ZOOM_VALUE.Val_12) #12倍
                                            ...
                                            #フライトプラン再開
                                            self.droneControlAction("fp_resume")
                                        elif gbl_data_from_telemetry.objectdetection_tower_mode == CST.OBJECTDETECTION_TOWER_MODE.kensui_500kv:
                                            self.cameraControlZoom(CST.ZOOM_VALUE.Val_12) #12倍
                                            self.cameraControlFocus(2,0.5,0.5)  #AFC、中心
                                            generate_sampling_group_data.clear()   #サンプリングデータ初期化
                                            #カメラ・ジンバル制御待機(画面描画)
                                            ...
                                            if gbl_instruction_from_click.currentCameraLens == CST.CAMERALENS.wide:
                                                ...
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_12) #12倍？ #107
                                                self.cameraControlFocus(2, 0.5, 0.5)  #AFC、中心
                                                generate_sampling_group_data.clear()   #サンプリングデータ初期化
                                                ...
                                                generate_sampling_group_data.clear()   #サンプリングデータ初期化
                                                gbl_data_from_telemetry.flg_objectdetection_auto = False
                                                gbl_save_prev_data.flg_objectdetection_auto_prev = False  #test20220404
                                            ...
                                            if (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_2):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_2)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_4):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_4)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_5):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_5)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_6):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_6)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_8):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_8)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_10):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_10)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_12):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_12)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_16):
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_16)
                                            elif (gbl_gimbal_instruction_from_detection.zoomValueCurrent < CST.ZOOM_VALUE.Val_20):    #add 20220330
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_20)   #add 20220330
                                            else :
                                                self.cameraControlZoom(CST.ZOOM_VALUE.Val_2)
                                            #ズーム後フォーカス処理
                                            self.cameraControlFocus(2,0.5,0.5)  #AFC、中心
                ...
                #フラグ保持
                gbl_save_prev_data.flg_objectdetection_auto_prev = False  #test20220401
